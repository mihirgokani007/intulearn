...
public class SelectionSortExperiment extends ExperimentView {
    ...
    public SelectionSortExperiment(Context context) {
        super(context);
    ...
    protected ArrayList<Animator> newAnimation() {
        ...
        for (int i = 0; i < inputElements.size(); i++) {
            ...
            int minElement = Integer.parseInt(inputElements.get(i).getLabel());
            for (int k = i; k < inputElements.size(); k++) {
                if (minElement > Integer.parseInt(inputElements.get(k).getLabel())) {
                    minElement = Integer.parseInt(inputElements.get(k).getLabel());
                    minElementIndex = k;
                }
            }

            stepAnimation.add(getSelectedAnimation(minElementIndex));
            for (int j = minElementIndex - 1; j >= i; j--) {
                stepAnimation.add(getSlideAnimation(j, minElementIndex, minElementIndex - j));
            }
            stepAnimation.add(getDeselectedAnimation(minElementIndex));
            ...
            inputElements.add(i, inputElements.remove(minElementIndex));
        }
